...
WINDOW_SIZE = "980x640"
APP_TITLE = "Smart Home — Complete AI"
ASSISTANT_NAME = "Hey HOME"
...
# -------------------------
# Wake-word engine (porcupine or fallback)
# -------------------------
...
class SmartHomeApp:
    def __init__(self, master):
        ...
        # Assistant personality greeting
        self._assistant_greeting()
    ...
    def _assistant_greeting(self):
        ...
        if hour < 12:
            ...
            greet = "Good evening"
        responses = [
            f"{greet}, {ASSISTANT_NAME} here. Your smart home is online.",
            f"{greet}. I am {ASSISTANT_NAME}. Ready when you are.",
            f"Hello — {ASSISTANT_NAME} at your service. Say 'hey home' to wake me."
        ]
    ...
    def _build_ui(self):
        ...
        # Emergency quick control in UI
        tk.Button(controls, text="🚨 EMERGENCY", bg="#b91c1c", fg="white",
                  command=self._activate_emergency_ui).pack(pady=8)
    ...
    def _handle_command_text(self, text):
        ...
        if "turn off" in cmd and "fan" in cmd:
            ...
            return
        if "turn on" in cmd and "heater" in cmd:            # heater text command
            self.set_device("heater", True)
            return
        if "turn off" in cmd and "heater" in cmd:           # heater text command
            self.set_device("heater", False)
            return
    ...
    def _execute_canonical(self, canonical):
        if canonical == "turn on light":
        ...
        elif canonical == "turn on heater":                         
            self.set_device("heater", True)
        elif canonical == "turn off heater":                       
            self.set_device("heater", False)
            ...
            self.report_status()
        elif canonical == "emergency mode":
            self._activate_emergency_ui()
        else:
            self._log(f"Canonical command not implemented: {canonical}")

    # -------------------------
    # Device control + voice feedback
    # -------------------------
    def set_device(self, device_id, value):
        ...
        human_state = self._human_state(device_id)
        self._log(f"{device_id} -> {human_state}")
        self.tts.speak(f"{device_id.replace('_', ' ')} set to {human_state}")
    ...
    # -------------------------
    # Emergency - UI and voice
    # -------------------------
    def _activate_emergency_ui(self):
        # Emergency actions: turn off non-door devices and lock door
        self._log("EMERGENCY activated via UI")
        for d in DEVICES:
            if d["id"] != "door":
                self.device_state[d["id"]] = False
        # For security, lock the door
        self.device_state["door"] = False
        self._update_device_ui()
        save_state({"devices": self.device_state})
        self.tts.speak("Emergency mode activated. All non-essential devices are turned off and house locked.")
        messagebox.showwarning("EMERGENCY", "Emergency mode activated!")

    # -------------------------
    # Queue processing (voice / wake)
    # -------------------------
    def _process_queue(self):
        while not self.queue.empty():
            ...
            if typ == "transcript":
                ...
                # when wake detected, notify user and optionally start voice engine
                self.tts.speak("Yes?")
            else:
    ...
    def report_status(self):
        ...
        msg = " | ".join(parts)
        self._log(msg)
        # more detailed voice with assistant personality
        self.tts.speak(f"{ASSISTANT_NAME} reporting status. {msg}")
    ...
    def _on_wake_detected(self):
        # Put wake event into queue so _process_queue can log it and voice engine acts
        self.queue.put(("wake", True))
        # auto-start voice engine when wake detected (optional)
    ...
    def show_help(self):
        examples = (
            ...
            "- Status\n"
            "- Emergency\n\n"
            "(Voice features require SpeechRecognition and microphone; Wake-word supports Porcupine if installed.)"
        )
